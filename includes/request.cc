...
Request::Request(std::string_view request) : request_(request) {
    ...
    for(int i = 0; i < size_; i++){
        ...
        switch (state) {
            ...
            case kHttpType:
                if(i >= low + 3 && unit == '\n' && request[i-1] == '\r'){
                    http_version_ = std::string_view(request).substr(low, i - 1 - low);
                    low = i + 1;
                    i++;
                    state = kHeaderName;
                }
            ...
            case kHeaderValue:
                if(i >= low + 1 && unit == '\n' && request[i-1] == '\r'){
                    headers_[key_buffer] = std::string_view(request).substr(low, i - 1 - low);
                    key_buffer = "";
                    state = kHeaderName;
                    low = i + 1;
                }
    ...
    if(low < size_)
        body_ = std::string_view(request).substr(low, size_-low);
}
...
std::string_view Request::route(){
    return route_;
}

std::string_view Request::http_version(){
